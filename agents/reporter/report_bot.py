...
def generate_weekly_summary():
    ...
    data_file = None
    # Try combined weekly file first
    combined_file = project_root / "data" / f"combined-week-{week_tag}.json"
    if combined_file.exists():
        data_file = combined_file
    else:
        # Try individual weekly file
        weekly_file = project_root / "data" / f"week-{week_tag}.json"
        if weekly_file.exists():
            data_file = weekly_file
        else:
            # Fallback to general news
            data_file = project_root / "data" / "mit_ai_news.json"
    ...
    Keep summaries concise but informative. Response 5 sentence."""
